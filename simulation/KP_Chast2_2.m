% считывание эфемерид из файла corezh.nav на НКА с заданным номером
delimiterIn = ' ';
headerlinesIn = 5;
...
A = (sqrt_a)^2;
t = (toe+(84-time(nomer))*3600):0.1:(toe+(96-time(nomer))*3600);
N = size(t,2);
for k = 1:N
tk = t(k) - toe;
if tk > 302400
...
F = [x',y',z'];
f = fopen('C:\Users\Anna\Documents\Linux\LOGZZZ8\TestKP2.txt','w');
fprintf(f,'%f %f %f\n',F');
fclose(f);
hold on
